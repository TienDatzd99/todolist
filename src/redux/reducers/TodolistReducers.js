...
const initialState = {
  ...
  if (!localStorage.getItem('dataTaskNotComplete') || !localStorage.getItem('dataTaskComplete')) {
    // Nếu chưa có, lưu dữ liệu ban đầu vào localStorage
    localStorage.setItem('dataTaskNotComplete', JSON.stringify(initialState.dataTaskNotComplete));
    localStorage.setItem('dataTaskComplete', JSON.stringify(initialState.dataTaskComplete));
  }
  ...
  reducers: {
   addTaskAction: (state,action)=>{
    ...
    if (action.payload.completed === true) {
      // Cập nhật state
      const newState = { ...state, dataTaskComplete: action.payload };

      return newState;
    } else {
      // Cập nhật state
      const newState = { ...state, dataTaskNotComplete: action.payload };
